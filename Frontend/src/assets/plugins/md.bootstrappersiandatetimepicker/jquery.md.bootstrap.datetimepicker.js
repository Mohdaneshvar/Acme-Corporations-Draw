...
(function ($) {
  ...
  function jalCal(jy) {
    ...
    var leapG = div(gy, 4) - div((div(gy, 100) + 1) * 3, 4) - 150; // Determine the Gregorian date of Farvardin the 1st.
  ...
  var previousYearTextPersian = 'سال قبل',
    ...
    shortDayNamesPersian = ['ش', 'ی', 'د', 'س', 'چ', 'پ', 'ج'],
    shortDayNames = ['SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'],
    monthNamesPersian = ['فروردین', 'اردیبهشت', 'خرداد', 'تیر', 'مرداد', 'شهریور', 'مهر', 'آبان', 'آذر', 'دی', 'بهمن', 'اسفند'],
    monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
    weekDayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
    weekDayNamesPersian = ['یک شنبه', 'دوشنبه', 'سه شنبه', 'چهارشنبه', 'پنج شنبه', 'جمعه', 'شنبه']; //#endregion
  // #region Functions
  ...
  function getSelectedDateTimeTextFormatted(setting) {
    ...
    if (setting.rangeSelector && setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined) return getDateTimeString(!setting.isGregorian ? getDateTimeJsonPersian1(setting.rangeSelectorStartDate) : getDateTimeJson1(setting.rangeSelectorStartDate), setting.textFormat, setting.isGregorian, setting.englishNumber) + ' - ' + getDateTimeString(!setting.isGregorian ? getDateTimeJsonPersian1(setting.rangeSelectorEndDate) : getDateTimeJson1(setting.rangeSelectorEndDate), setting.textFormat, setting.isGregorian, setting.englishNumber);
  ...
  function getSelectedDateTimeFormatted(setting) {
    if (setting.selectedDate == undefined) return '';
    if (setting.rangeSelector && setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined) return getDateTimeString(getDateTimeJson1(setting.rangeSelectorStartDate), setting.dateFormat, setting.isGregorian, true) + ' - ' + getDateTimeString(getDateTimeJson1(setting.rangeSelectorEndDate), setting.dateFormat, setting.isGregorian, true);
  ...
  function getAmPm(hour, isGregorian) {
    ...
    if (hour > 12) {
      if (isGregorian) amPm = 'PM';
      else amPm = 'ب.ظ';
    } else if (isGregorian) amPm = 'AM';
    else amPm = 'ق.ظ';
  ...
  function getDateTime4(dateNumber, dateTime, setting) {
    ...
    } else dateTime = new Date(dateTimeJson.year, dateTimeJson.month - 1, dateTimeJson.day);
  ...
  function getDaysInMonthPersian(year, month) {
    ...
    if (month > 6 && month < 12) numberOfDaysInMonth = 30;
    else if (month == 12) numberOfDaysInMonth = isLeapYear(year) ? 30 : 29;
  ...
  function getDateTimeString(dateTimeJson, format, isGregorian, englishNumber) {
    if (isGregorian) englishNumber = true; /// فرمت های که پشتیبانی می شوند
    /// <para />
    /// yyyy: سال چهار رقمی
    /// <para />
    /// yy: سال دو رقمی
  ...
  function parsePersianDateTime(persianDateTimeInString, dateSeperatorPattern) {
    ...
    persianDateTimeInString = '-' + persianDateTimeInString + '-'; // بدست آوردن ب.ظ یا ق.ظ

    if (persianDateTimeInString.indexOf('ق.ظ') > -1) amPmEnum = amPmEnum.AM;
    else if (persianDateTimeInString.indexOf('ب.ظ') > -1) amPmEnum = amPmEnum.PM;
    ...
    if (persianDateTimeInString.indexOf(':') > -1) // رشته ورودی شامل ساعت نیز هست
    {
      ...
      if (minuteAndSecondAndMiliSecondMatch[1] != undefined) second = minuteAndSecondAndMiliSecondMatch[1].replace(/\D+/, '');
      if (minuteAndSecondAndMiliSecondMatch[2] != undefined) miliSecond = minuteAndSecondAndMiliSecondMatch[2].replace(/\D+/, '');
    }

    if (containMonthSeperator) {
      var monthDayMath = persianDateTimeInString.match(/-\d{1,2}(?=-\d{1,2}[^:]|-)/img); // بدست آوردن ماه

      month = monthDayMath[0].replace(/\D+/, ''); // بدست آوردن روز

      day = monthDayMath[1].replace(/\D+/, ''); // بدست آوردن سال
      ...
      if (yearMatch != null) year = yearMatch[0].replace(/\D+/, '');
      else {
    ...
    if (numericYear <= 0) numericYear = persianDateTime[0];
    if (numericMonth <= 0) numericMonth = persianDateTime[1];
    if (numericDay <= 0) numericDay = persianDateTime[2];
  ...
  function getYearsToSelectHtml(setting, yearToStart) {
    ...
    for (var i = yearStart; i < yearEnd; i++) {
      ...
      if (disableBeforeDateTimeJson != undefined && disableBeforeDateTimeJson.year != undefined && currentYearDateTimeNumber < convertToNumber1(disableBeforeDateTimeJson)) currentYearDisabledAttr = 'disabled';
      if (disableAfterDateTimeJson != undefined && disableAfterDateTimeJson.year != undefined && currentYearDateTimeNumber < convertToNumber1(disableAfterDateTimeJson)) currentYearDisabledAttr = 'disabled';
      if (setting.disableBeforeToday && currentYearDateTimeNumber < convertToNumber1(todayDateTimeJson)) currentYearDisabledAttr = 'disabled';
      if (setting.disableAfterToday && currentYearDateTimeNumber > convertToNumber1(todayDateTimeJson)) currentYearDisabledAttr = 'disabled';
      if (counter == 1) yearsToSelectHtml += '<tr>';
      yearsToSelectHtml += "\n<td class=\"text-center\" ".concat(selectedDateTimeToShowJson.year == i ? 'selected-year' : '', ">\n    <button class=\"btn btn-sm btn-light\" type=\"button\" data-changedatebutton data-number=\"").concat(yearDateNumber, "\" ").concat(currentYearDisabledAttr, ">").concat(yearText, "</button>        \n</td>\n");
  ...
  function getDateTimePickerHtml(setting) {
    ...
    if (setting.rangeSelector && rangeSelectorStartDateJson != undefined && rangeSelectorEndDateJson != undefined) {
      selectedDateString = "".concat(getWeekDayName(rangeSelectorStartDateJson.dayOfWeek, setting.isGregorian), "\u060C ").concat(rangeSelectorStartDateJson.day, " ").concat(getMonthName(rangeSelectorStartDateJson.month - 1, setting.isGregorian), " ").concat(rangeSelectorStartDateJson.year, " - \n                ").concat(getWeekDayName(rangeSelectorEndDateJson.dayOfWeek, setting.isGregorian), "\u060C ").concat(rangeSelectorEndDateJson.day, " ").concat(getMonthName(rangeSelectorEndDateJson.month - 1, setting.isGregorian), " ").concat(rangeSelectorEndDateJson.year);
    } else selectedDateString = "".concat(getWeekDayName(selectedDateTimeJson.dayOfWeek, setting.isGregorian), "\u060C ").concat(selectedDateTimeJson.day, " ").concat(getMonthName(selectedDateTimeJson.month - 1, setting.isGregorian), " ").concat(selectedDateTimeJson.year);

    todayDateString = "".concat(setting.isGregorian ? 'Today,' : 'امروز،', " ").concat(todayDateTimeJson.day, " ").concat(getMonthName(todayDateTimeJson.month - 1, setting.isGregorian), " ").concat(todayDateTimeJson.year);
    ...
    if (disableAfterDateTimeJson != undefined && disableAfterDateTimeJson.year <= selectedDateTimeToShowJson.year && disableAfterDateTimeJson.month < selectedDateTimeToShowJson.month) selectedDateToShow = setting.isGregorian ? new Date(disableAfterDateTimeJson.year, disableAfterDateTimeJson.month - 1, 1) : getDateTime1(disableAfterDateTimeJson.year, disableAfterDateTimeJson.month, disableAfterDateTimeJson.day);
    if (disableBeforeDateTimeJson != undefined && disableBeforeDateTimeJson.year >= selectedDateTimeToShowJson.year && disableBeforeDateTimeJson.month > selectedDateTimeToShowJson.month) selectedDateToShow = setting.isGregorian ? new Date(disableBeforeDateTimeJson.year, disableBeforeDateTimeJson.month - 1, 1) : getDateTime1(disableBeforeDateTimeJson.year, disableBeforeDateTimeJson.month, disableBeforeDateTimeJson.day);
    var monthsTdHtml = '',
  ...
  function getDateTimePickerMonthHtml1(setting, isNextMonth, isPrevMonth) {
    ...
    if (firstWeekDayNumber != 6) {
      ...
      for (i = numberOfDaysInPreviousMonth - firstWeekDayNumber; i <= numberOfDaysInPreviousMonth; i++) {
        ...
        $td = $('<td data-nm />').attr('data-number', currentDateNumber).html(dayNumberInString);

        if (setting.rangeSelector) {
          if (currentDateNumber == rangeSelectorStartDateNumber || currentDateNumber == rangeSelectorEndDateNumber) $td.addClass('selected-range-days-start-end');
          else if (rangeSelectorStartDateNumber > 0 && rangeSelectorEndDateNumber > 0 && currentDateNumber > rangeSelectorStartDateNumber && currentDateNumber < rangeSelectorEndDateNumber) $td.addClass('selected-range-days');
        } // روز جمعه


        if (!setting.isGregorian && tdNumber == 6) $td.addClass('text-danger'); // روز یکشنبه
        else if (setting.isGregorian && tdNumber == 0) $td.addClass('text-danger');
        $tr.append($td);
        cellNumber++;
        tdNumber++;
    ...
    for (i = 1; i <= numberOfDaysInCurrentMonth; i++) {
      ...
      dayNumberInString = setting.englishNumber ? zeroPad(i) : toPersianNumber(zeroPad(i));
      $td = $('<td data-day />').attr('data-number', currentDateNumber).html(dayNumberInString); // امروز

      if (currentDateNumber == todayDateNumber) {
        $td.attr('data-today', ''); // اگر نام روز هفته انتخاب شده در تکس باکس قبل از تاریخ امروز باشد
        // نباید دیگر نام روز هفته تغییر کند

        if (!dayOfWeek) dayOfWeek = getWeekDayName(tdNumber - 1 < 0 ? 0 : tdNumber - 1, setting.isGregorian);
      ...
      if (setting.disableBeforeToday) {
        ...
        if (nextMonthDateNumber < todayDateNumber) nextMonthButtonDisabledAttribute = 'disabled';
        if (nextYearDateNumber < todayDateNumber) nextYearButtonDisabledAttribute = 'disabled';
        if (previousMonthDateNumber < todayDateNumber) previousMonthButtonDisabledAttribute = 'disabled';
        if (previousYearDateNumber < todayDateNumber) previousYearButtonDisabledAttribute = 'disabled';

        for (j = 1; j <= 12; j++) {
          if (monthsDateNumberAndAttr['month' + j.toString() + 'DateNumber'] < todayDateNumber) monthsDateNumberAndAttr['selectMonth' + j.toString() + 'ButtonCssClass'] = 'disabled';
        }
      }
      ...
      if (disableAfterDateTimeNumber) {
        ...
        if (nextMonthDateNumber > disableAfterDateTimeNumber) nextMonthButtonDisabledAttribute = 'disabled';
        if (nextYearDateNumber > disableAfterDateTimeNumber) nextYearButtonDisabledAttribute = 'disabled';
        if (previousMonthDateNumber > disableAfterDateTimeNumber) previousMonthButtonDisabledAttribute = 'disabled';
        if (previousYearDateNumber > disableAfterDateTimeNumber) previousYearButtonDisabledAttribute = 'disabled';

        for (j = 1; j <= 12; j++) {
          if (monthsDateNumberAndAttr['month' + j.toString() + 'DateNumber'] > disableAfterDateTimeNumber) monthsDateNumberAndAttr['selectMonth' + j.toString() + 'ButtonCssClass'] = 'disabled';
        }
      }

      if (disableBeforeDateTimeNumber) {
        if (currentDateNumber < disableBeforeDateTimeNumber) $td.attr('disabled', '');
        if (nextMonthDateNumber < disableBeforeDateTimeNumber) nextMonthButtonDisabledAttribute = 'disabled';
        if (nextYearDateNumber < disableBeforeDateTimeNumber) nextYearButtonDisabledAttribute = 'disabled';
        if (previousMonthDateNumber < disableBeforeDateTimeNumber) previousMonthButtonDisabledAttribute = 'disabled';
        if (previousYearDateNumber < disableBeforeDateTimeNumber) previousYearButtonDisabledAttribute = 'disabled';

        for (j = 1; j <= 12; j++) {
          if (monthsDateNumberAndAttr['month' + j.toString() + 'DateNumber'] < disableBeforeDateTimeNumber) monthsDateNumberAndAttr['selectMonth' + j.toString() + 'ButtonCssClass'] = 'disabled';
        }
      }

      for (j = 0; j < disabledDatesNumber.length; j++) {
        if (currentDateNumber == disabledDatesNumber[j]) $td.attr('disabled', '');
      }
  ...
  $(document).on('click', mdDatePickerContainerSelector + ' [data-day]', function () {
    ...
    if (setting.rangeSelector) {
      ...
      if (setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined) {
        ...
        $this.parents('table:last').find('td.selected-range-days-start-end,td.selected-range-days').removeClass('selected-range-days').removeClass('selected-range-days-start-end');
      }
  ...
  $(document).on('click', mdDatePickerContainerSelector + ' [select-year-button]', function () {
    var $this = $(this),
      ...
      yearsRangeText = " ".concat(yearsToSelectObject.yearStart, " - ").concat(yearsToSelectObject.yearEnd, " "),
  ...
  var methods = {
    init: function init(options) {
      return this.each(function () {
        ...
        $(document).on('change', setting.targetTextSelector, function () {
          ...
          try {
            if (!setting.rangeSelector) $this.MdPersianDateTimePicker('setDate', parseDateTime(value1, setting));
            else {
              var dateValues = value1.split(' - ');
    ...
    setDate: function setDate(dateTimeObject) {
    ...
    setDateRange: function setDateRange(startDateTimeObject, endDateTimeObject) {
    ...
    setDatePersian: function setDatePersian(dateTimeObjectJson) {
    ...
    disable: function disable(isDisable) {
      return this.each(function () {
        var $this = $(this),
          setting = getSetting2($this);
    ...
    destroy: function destroy() {
      return this.each(function () {
        ...
        $this.removeAttr(mdDatePickerFlag).removeAttr('data-toDate').removeAttr('data-fromDate');
        $this.off(setting.trigger).popover('dispose'); //if ($(mdDatePickerFlagSelector).length <= 1) {
    ...
    changeType: function changeType(isGregorian, englishNumber) {
      return this.each(function () {
        var $this = $(this),
          setting = getSetting2($this);
        hidePopover($this);
