...
export class DatePickerComponent implements OnInit {
    ...
    changeValue(): void {
      var valueFa= this.dateTimeService.getPersianDate();
     const validMaxyear = Number(valueFa.substr(0, 4));
     const validMinyear = validMaxyear-150;
     const validMaxmonth = Number(valueFa.substr(5, 2));

        if (this.year && this.month && this.day && this.year>validMinyear && (this.year < validMaxyear ||(this.year == validMaxyear && this.month <= validMaxmonth ))) {
    ...
    fixYear(): void {
        if (this.year && this.year.toString().length === 2) {
            const y: number = 13;//+this.dateTimeService.getPersianDate('jYYYY').substr(0, 2);
