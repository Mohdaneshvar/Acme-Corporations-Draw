...
function addErrorsToForm(formId: string, errors: string[]): void {
    if (formId !== '') {
        ...
        h6.innerHTML = '<i class="mdi mdi-alert-octagon mdi-24px"></i> Please check these field';
        $(div).append(h6);
...
export function NationalIdValidator(err: string): ValidatorFn {
    return (control: AbstractControl): ValidationErrors => {
        if (control.value === null || control.value.toString() === '' ||control.value.toString().length==11) {
            return null;
...
export function IsStringaEnglishOrNumberCharacterValidator(err: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors => {
...
export function ProductSerialValidator(err: string): ValidatorFn {
    return (control: AbstractControl): ValidationErrors => {
        if (control.value === null || control.value.toString() === '') {
            return null;
        //MUYVD-LSEBY-CXHRQ-XFAGY
        //
    
    } else if (/^[A-Za-z]{5}-[A-Za-z]{5}-[A-Za-z]{5}-[A-Za-z]{5}$/.test(control.value.toString())) {
            return null;
        } else {
            return { error: err };
        }
...
function convertNumber(val: number | string): number | string {
 ...
 export function ForbiddenCharValidator(err: string): ValidatorFn {
  return (control: AbstractControl): ValidationErrors => {
      if (control.value === null || control.value.toString() === '') {
          return null;
      }
          const chars:string[]=['~','!','@','#','$','%','^','&','*','(',')','/','\\','|','.','-','+',',',':',';','`','[',']','{','}','>','<','?'];
       if (!chars.some(e =>control.value.toString().includes(e))) {
           return null;
       } else {
         return { error: err };
       }
